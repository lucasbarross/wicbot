...
module.exports.getHint = async (playerID, hash, lang) => {
  try {
    await checkToken();
    return axios.get(
      `/api/v1/localizations/hint?hash=${hash}&lang=${lang}&player_id=${playerID}`
    );
  } catch (err) {
    return Promise.reject(err);
  }
...
async function checkToken() {
  return new Promise((resolve, reject) => {
    if (
      !this.authorization ||
      this.authorization.created_at + this.authorization.expires_in <=
      ...
      getToken()
        ...
        .catch((err) =>
          reject(new Error("Couldn't get authorization token: " + err.message))
        );
    } else {
